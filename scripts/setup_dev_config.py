...
# Ollama image for node-manager to start inference. Use the ROCm variant for AMD GPUs;
# override with OLLAMA_IMAGE env var (e.g. for CPU-only or NVIDIA nodes).
OLLAMA_IMAGE = os.environ.get("OLLAMA_IMAGE", "ollama/ollama:rocm")
...
NODE_MANAGER_BIN = os.path.join(PROJECT_ROOT, "worker_node", "bin", "cynodeai-wnm-dev")
...
def compose_env():
    ...
    # OLLAMA_BASE_URL: allow env override so user-gateway can reach a host Ollama container
    # (started by node-manager) when Ollama is NOT in the compose network.
    # Default is the compose service name used when ollama profile is active.
    ollama_base_url = os.environ.get(
        "OLLAMA_BASE_URL",
        f"http://{CONTAINER_HOST_ALIAS}:11434",
    )
    return {
