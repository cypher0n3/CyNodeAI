...
4. Set `requirements` (optional) using requirement objects per skill `pma-requirement-object`.
5. Set `steps` (required, non-empty map): numeric IDs (10, 20, 30, …), each value `{ "complete": false, "description": "<string>" }` when creating.
6. Set `post_execution_notes` or `comments` when appropriate (e.g. after execution or when adding comments).
