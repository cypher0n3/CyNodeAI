...
def clean_ollama_data():
    """Remove the ollama data volume if present (created/managed by CyNodeAI compose)."""
    if not _cfg.ensure_runtime():
        return False
    rt = _cfg.RUNTIME
    # Compose project name is the dir containing the compose file (orchestrator).
    compose_dir = os.path.dirname(_cfg.COMPOSE_FILE)
    project = os.path.basename(compose_dir)
    volume_name = f"{project}_ollama-data"
    log_info("Cleaning up Ollama data volume (if present)...")
    subprocess.run(
        [rt, "volume", "rm", volume_name],
        capture_output=True, check=False, shell=False,
    )
    return True
...
def start_node(extra_env=None):
    ...
    # Ollama image for GPU inference; node-manager uses this when starting Ollama.
    if not env.get("OLLAMA_IMAGE"):
        env["OLLAMA_IMAGE"] = _cfg.OLLAMA_IMAGE
    ...
    node_log_path = os.path.join(_cfg.LOGS_DIR, "cynodeai-wnm.log")
    try:
...
def _wait_pid_exit(pid, timeout_sec=15):
    """Poll until process *pid* has exited or *timeout_sec* elapses. Returns True when gone."""
...
def _kill_stale_wnm_processes():
    """Kill any running WNM or worker-api processes not tracked by the PID file.

    Each call to setup-dev start launches a new WNM process and overwrites the PID file,
    leaving previous instances running. This sweeps them all up by binary name.
    """
    for bin_name in (
        os.path.basename(_cfg.NODE_MANAGER_BIN),
        os.path.basename(_cfg.NODE_MANAGER_WORKER_API_BIN),
    ):
        try:
            r = subprocess.run(
                ["pgrep", "-f", bin_name],
...
def stop_node(stop_managed_containers=True):
    """Stop node-manager (SIGTERM then SIGKILL), free worker port.

    Optionally stop managed containers. Best-effort; does not raise. Returns True when
    node-manager was not running or stopped cleanly. Kills all WNM and worker-api by
    binary name to catch stale processes from previous runs where PID file was
    overwritten without stopping the old instance.
    """
    # Kill all matching processes by binary name first (handles stale instances).
    _kill_stale_wnm_processes()
    ...
    if pid is not None:
        if not _wait_pid_exit(pid, timeout_sec=15):
            log_warn("node-manager did not exit within 15s; sending SIGKILL")
            try:
                os.kill(pid, signal.SIGKILL)
            except ProcessLookupError:
                pass
            _wait_pid_exit(pid, timeout_sec=5)
    try:
    ...
    if stop_managed_containers and _cfg.ensure_runtime():
        for name_filter in ["cynodeai-managed-", "cynodeai-worker-api"]:
            r = subprocess.run(
                [_cfg.RUNTIME, "ps", "-aq", "--filter", f"name={name_filter}"],
                capture_output=True, text=True, timeout=10, check=False, shell=False,
            )
            for cid in (r.stdout or "").strip().split():
                subprocess.run(
                    [_cfg.RUNTIME, "stop", "--time", "5", cid],
...
def stop_all(leave_ollama_running=False):
    """Kill node-manager, wait for it to stop its managed containers, then compose down.

    Shutdown order:
      1. Capture container logs while everything is still running.
      2. SIGTERM node-manager and wait up to 15 s for it to exit (it will stop its containers).
      3. Stop any remaining cynodeai-* containers that node-manager may have left behind.
      4. Kill anything still on the worker port.
      5. Compose down orchestrator stack.
      6. Force-remove any named well-known containers that may have been missed.

    If leave_ollama_running, restart ollama after compose down.
    Best-effort: never raises so caller can still report success (e.g. after E2E pass).
    """

    def stop_node_managed_pods():
        """Stop and remove any pods that node-manager created (e.g. cynodeai-worker-api-pod-*)."""
        if not _cfg.ensure_runtime():
            return
        r = subprocess.run(
            [_cfg.RUNTIME, "pod", "ls", "--format", "{{.Name}}"],
            capture_output=True, text=True, timeout=10, check=False, shell=False,
        )
        for pod in (r.stdout or "").strip().splitlines():
            pod = pod.strip()
            if pod.startswith("cynodeai-"):
                subprocess.run(
                    [_cfg.RUNTIME, "pod", "stop", pod],
                    capture_output=True, timeout=15, check=False, shell=False,
                )
                subprocess.run(
                    [_cfg.RUNTIME, "pod", "rm", "-f", pod],
                    capture_output=True, timeout=10, check=False, shell=False,
                )
    ...
    try:
        ...
        _stop_all_step(
            "kill node-manager (wait for exit)",
            lambda: stop_node(stop_managed_containers=True),
        )
        _stop_all_step("stop node-managed pods", stop_node_managed_pods)
        ...
        _stop_all_step("rm named containers", rm_named_containers)
