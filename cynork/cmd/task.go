...
func printTaskResult(result *userapi.TaskResultResponse) {
	if outputFmt == outputFormatJSON {
		printTaskResultJSON(result)
		return
	}
	printTaskResultText(result)
}

func printTaskResultJSON(result *userapi.TaskResultResponse) {
	...
	for _, j := range result.Jobs {
		...
		stdout.WriteString(*j.Result)
		if jobStderr, parsed := parseTaskJobResultStderr(*j.Result); parsed {
			stderr.WriteString(jobStderr)
		}
	}
	if terminalTaskStatuses[result.Status] || stdout.Len() > 0 {
		out["stdout"] = stdout.String()
	}
	if terminalTaskStatuses[result.Status] || stderr.Len() > 0 {
		out["stderr"] = stderr.String()
	}
	_ = jsonOutputEncoder().Encode(out)
}

func printTaskResultText(result *userapi.TaskResultResponse) {
...
func parseTaskJobResultStderr(raw string) (string, bool) {
	var parsed struct {
