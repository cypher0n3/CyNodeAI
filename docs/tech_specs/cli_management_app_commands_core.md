...
- If `-u` / `--user` is not provided, the CLI MUST prompt `User:` on stderr and read one line from stdin.
- If `--password-stdin` is set, the CLI MUST require `-u` / `--user` to be provided.
...
- Table mode MUST print exactly one line containing `logged_in=true` and `user=<user>`.
- JSON mode MUST print `{"logged_in":true,"user":"<user>"}`.
...
- Table mode MUST print exactly one line containing `id=<id>` and `user=<user>`.
- JSON mode MUST print `{"id":"<id>","user":"<user>"}`.
