...
- [NATS Transport](#nats-transport)
  - [NATS Connection Lifecycle](#nats-connection-lifecycle)
  - [Session Lifecycle over NATS](#session-lifecycle-over-nats)
  - [Chat Streaming over NATS](#chat-streaming-over-nats)
  - [NATS Reconnect and Fallback](#nats-reconnect-and-fallback)
  - [NATS Transport Traces To](#nats-transport-traces-to)
...
- Spec ID: `CYNAI.CLIENT.CynorkTui.NatsTransport` <a id="spec-cynai-client-cynorktui-natstransport"></a>

cynork connects to NATS directly after authenticating via the HTTP API.
NATS replaces HTTP/SSE as the primary transport for session lifecycle signaling and chat streaming.
...
1. cynork authenticates via `POST /v1/auth/login` through the user-gateway (existing HTTP flow).
2. The login response includes a `nats` configuration block containing the server URL, session-scoped JWT, JWT expiry, and optional TLS CA.
   cynork MUST NOT hardcode or cache NATS connection details across sessions; each login yields the current configuration from the orchestrator.
   See [CYNAI.USRGWY.NatsClientCredentials](../nats_messaging.md#spec-cynai-usrgwy-natsclientcredentials) for the full schema.
3. cynork connects to the NATS server using the URL and JWT from the `nats` block (TCP).
4. On successful NATS connection, cynork publishes `session.attached` to `cynode.session.attached.<tenant_id>.<session_id>`.
5. On logout or clean exit, cynork publishes `session.detached` to `cynode.session.detached.<tenant_id>.<session_id>` with reason `logout` or `client_close`, then disconnects from NATS.
6. On shutdown, cynork closes the NATS connection gracefully.

The NATS JWT subject permissions restrict cynork to its own session subjects only.

### Session Lifecycle Over NATS

- cynork publishes `session.activity` to `cynode.session.activity.<tenant_id>.<session_id>` at `T_heartbeat` cadence while the user is interacting (keystrokes, commands, chat messages).
- cynork publishes `session.attached` on initial NATS connection and on reconnection after an idle period.
- cynork publishes `session.detached` on logout, clean exit, or explicit session close.
- If cynork crashes, the absence of `session.activity` messages within the idle window serves as implicit detach on the server side.

### Chat Streaming Over NATS

NATS replaces the HTTP/SSE chat streaming path.
The token delta model (visible text, thinking, tool calls, amendments) remains the same; only the transport changes from SSE events to NATS messages.

- To send a chat message, cynork publishes a `chat.request` message to `cynode.chat.request.<session_id>` containing the chat completion payload (messages, model, options).
- cynork subscribes to `cynode.chat.stream.<session_id>.>` for token deltas, `cynode.chat.done.<session_id>.>` for stream completion, and `cynode.chat.amendment.<session_id>.>` for post-stream redaction amendments.
- Token deltas arrive as `chat.stream` NATS messages with a `sequence` field for ordering.
  The TUI applies each delta to the in-flight assistant turn using the same rendering logic as the current SSE path (`applyStreamDelta`, thinking buffer, tool-call buffer).
- Stream completion arrives as a `chat.done` NATS message with `finish_reason` and `usage`.
  The TUI commits the in-flight turn and removes the progress indicator.
- Amendments arrive as `chat.amendment` NATS messages.
  The TUI replaces the accumulated visible text for the in-flight turn with the amended content, identical to the current `cynodeai.amendment` SSE event handling.
- The `message_id` field in `chat.request` is generated by cynork and used as the subject token in all response messages, allowing cynork to correlate responses to requests.

### NATS Reconnect and Fallback

- If the NATS connection drops, cynork attempts reconnection with bounded exponential backoff (same strategy as HTTP connection recovery in [CYNAI.CLIENT.CynorkTui.ConnectionRecovery](#spec-cynai-client-cynorktui-connectionrecovery)).
- During NATS disconnection, session activity publishing pauses; it resumes on reconnection.
- If a chat stream is in progress when the NATS connection drops, the TUI marks the in-flight turn as interrupted and preserves already-received content.
  On reconnection, the client can replay missed messages from the `CYNODE_CHAT` JetStream stream using the last received `sequence` number.
- cynork does NOT fall back to HTTP/SSE for chat streaming.
  If NATS is unavailable, the TUI displays a connection status indicator and waits for reconnection.

### NATS Transport Traces To
