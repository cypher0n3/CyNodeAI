...
Direct tool names MUST match tools offered by that server (or the [MCP tool specifications](../tech_specs/mcp_tools/README.md) when `Server` is `default`); argument keys and types follow the catalog and [common argument requirements](../tech_specs/mcp/mcp_tooling.md#spec-cynai-mcptoo-commonargumentrequirements).
Each tool definition MUST declare **agent scope** (which agents may invoke it), consistent with [Per-tool scope: Sandbox vs PM](../tech_specs/mcp_tools/access_allowlists_and_scope.md#spec-cynai-mcpgat-pertoolscope).
...
- **External**: User-installed or remote MCP tools; registered via [User-Installable MCP Tools](../tech_specs/mcp/user_installable_mcp_tools.md); `Server` is an API endpoint registered with the API gateway.
...
- [MCP tool specifications](../tech_specs/mcp_tools/README.md)
- [MCP tooling](../tech_specs/mcp/mcp_tooling.md)
- [MCP gateway enforcement](../tech_specs/mcp/mcp_gateway_enforcement.md)
- [User-installable MCP tools](../tech_specs/mcp/user_installable_mcp_tools.md)
...
- **API gateway endpoint registration** and **same endpoint, different users** are specified in the draft [MCP Endpoint Registry](mcp_endpoint_registry_spec.md).
  That draft defines: the endpoint record model (slug, base_url, credential_ref, owner, scope); per-user vs shared endpoints; RBAC actions (`mcp.endpoint.create/read/update/delete/use`); the User API Gateway registration API (`/v1/mcp/endpoints`); and resolution of `Server` to (base_url, credentials) in request context.
  Once the draft is adopted and the User API Gateway spec is updated to reference it, this "Dependencies and gaps" subsection may be reduced to a pointer to the endpoint registry spec.
...
#### 3.2.1 Fields (Direct Call)

- `Name`: MCP tool name on the parent's `Server` (e.g. `artifact.get`, `project.get`); MUST match a tool offered by that server (or the [MCP tool specifications](../tech_specs/mcp_tools/README.md) when `Server` is `default`).
...
#### 3.2.2 Fields (Sub-Tool Reference)
...
MUST align with [Per-tool scope: Sandbox vs PM](../tech_specs/mcp_tools/access_allowlists_and_scope.md#spec-cynai-mcpgat-pertoolscope): the gateway uses this value in addition to role-based allowlists to allow or deny tool calls.
...
- Table name: `mcp_tool_definitions` (or as defined in [postgres_schema](../tech_specs/postgres_schema.md) when the table is added).
...
Scope is persisted and enforced by the MCP gateway per [Per-tool scope: Sandbox vs PM](../tech_specs/mcp_tools/access_allowlists_and_scope.md#spec-cynai-mcpgat-pertoolscope).
...
- User-installed external tools follow the same scope model; see [User-Installable MCP Tools](../tech_specs/mcp/user_installable_mcp_tools.md).
...
Relationship to tool definitions
