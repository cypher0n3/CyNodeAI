...
func TestGetTaskByNameNotFound(t *testing.T) {
	...
	tests := []struct {
		...
		{"GetTask", "/v1/tasks/unknown-task-name", handler.GetTask},
		{"GetTaskResult", "/v1/tasks/unknown-task-name/result", handler.GetTaskResult},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			req, rec := recordedRequest("GET", tt.path, nil)
			req.SetPathValue("id", "unknown-task-name")
			ctx := context.WithValue(req.Context(), contextKeyUserID, userID)
			tt.handle(rec, req.WithContext(ctx))
			assertStatusCode(t, rec, http.StatusNotFound)
		})
	}
}

func TestGetTaskByNameResolvesCorrectly(t *testing.T) {
	mockDB := testutil.NewMockDB()
	handler := NewTaskHandler(mockDB, slog.Default(), "", "")
	ctx := context.Background()
	userID := uuid.New()
	taskName := "my-task"
	task, _ := mockDB.CreateTask(ctx, &userID, "some prompt", &taskName)

	req, rec := recordedRequest("GET", "/v1/tasks/my-task", nil)
	req.SetPathValue("id", "my-task")
	reqCtx := context.WithValue(req.Context(), contextKeyUserID, userID)
	handler.GetTask(rec, req.WithContext(reqCtx))
	assertStatusCode(t, rec, http.StatusOK)
	if !strings.Contains(rec.Body.String(), task.ID.String()) {
		t.Errorf("response should contain task ID %s: %s", task.ID, rec.Body.String())
	}
}
...
func TestBuildSBAJobPayload(t *testing.T) {
	...
	payload, err := buildSBAJobPayload(taskID, jobID, "test prompt", "qwen3.5:0.8b")
	if err != nil {
