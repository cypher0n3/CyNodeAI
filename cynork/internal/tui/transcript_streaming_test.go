...
func TestReadNextDelta_MapsChatDeltas(t *testing.T) {
	...
	ch <- chat.ChatStreamDelta{Amendment: "amend"}
	if m, ok := readNextDelta(ch).(streamDeltaMsg); !ok || m.amendment != "amend" {
		t.Fatalf("amend: %+v", m)
	}
	ch <- chat.ChatStreamDelta{Thinking: "why"}
	if m, ok := readNextDelta(ch).(streamDeltaMsg); !ok || m.thinking != "why" {
		t.Fatalf("think: %+v", m)
	}
	ch <- chat.ChatStreamDelta{ToolName: "fn", ToolArgs: "{}"}
	if m, ok := readNextDelta(ch).(streamDeltaMsg); !ok || m.toolName != "fn" {
		t.Fatalf("tool: %+v", m)
	}
	ch <- chat.ChatStreamDelta{IsHeartbeat: true, HeartbeatElapsed: 2, HeartbeatStatus: "wait"}
	if m, ok := readNextDelta(ch).(streamDeltaMsg); !ok || !m.isHeartbeat {
		t.Fatalf("hb: %+v", m)
	}
	ch <- chat.ChatStreamDelta{IterationStart: true, Iteration: 3}
	if m, ok := readNextDelta(ch).(streamDeltaMsg); !ok || !m.iterationStart || m.iteration != 3 {
		t.Fatalf("iter: %+v", m)
	}
	ch <- chat.ChatStreamDelta{Delta: "text"}
	if m, ok := readNextDelta(ch).(streamDeltaMsg); !ok || m.delta != "text" {
		t.Fatalf("delta: %+v", m)
	}
}
