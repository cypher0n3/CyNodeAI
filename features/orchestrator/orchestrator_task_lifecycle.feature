...
Scenario: Task with input_mode commands runs literal shell
  When I create a task with input_mode "commands" and prompt "echo hello" and the orchestrator selects the node for dispatch
  Then the job sent to the worker has command containing "echo hello"

@req_orches_0121
@spec_cynai_orches_doc_orchestrator
Scenario: List tasks as authenticated user
  When I create a task with prompt "list me" and I list tasks with limit 10
