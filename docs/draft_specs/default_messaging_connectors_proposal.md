...
Implementation details (webhook ingestion, idempotency, replay protection) SHOULD align with the existing [Connector Framework](../tech_specs/connector_framework.md) and [connector hardening draft](connector_framework_hardening.md) (e.g. webhook signature validation, replay window).
...
- **Format:** Commands start with a slash (e.g. `/setting`, `/config`, `/help`, `/status`, `/task cancel`, `/kill job`).
  ...
  - Example: `/task cancel <task_id>` or `/kill job <task_id>` to cancel a task and stop its running job at user direction; PMA invokes the orchestrator cancel path and the worker node stops the SBA (graceful then container kill fallback).
    See [User-Directed Job Kill](user_directed_job_kill_proposal.md).
...
- [Connector Framework Hardening (draft)](connector_framework_hardening.md)
- [User-Directed Job Kill (draft)](user_directed_job_kill_proposal.md)
