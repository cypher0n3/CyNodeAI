# Failed E2E Report: e2e_0520_task_prompt.test_prompt_task

## 1 Summary

Test `e2e_0520_task_prompt.TestPromptTask.test_prompt_task` failed because no `task_id` was obtained from `cynork task create` after up to 3 attempts.
...
- **Root cause:** Same as e2e_0420: task create did not return a successful response within the test's retry window.
...
- [e2e_0520_task_prompt.py](../../../scripts/test_scripts/e2e_0520_task_prompt.py) lines 16-31: Loop up to 3 times running `cynork task create -p "What model are you? ..." -o json`, parse `task_id` from response; assert task_id is not None at line 31.
...
- User API Gateway and orchestrator: prompt-mode task create (REQ-ORCHES-0122, REQ-ORCHES-0126); interpretation and inference by default.
  Same create path as e2e_0420; response not returned within timeout in this run.
...
- **Deviation:** Same as e2e_0420: create path does not respond within 120 seconds or does not return a valid create response in the E2E environment.

## 6 What Needs to Be Fixed in the Implementation

- **Same root cause as e2e_0420.**
