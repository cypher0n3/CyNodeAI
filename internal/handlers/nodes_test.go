...
func TestRegisterBadRequest(t *testing.T) {
	...
	// Test invalid JSON
	req := httptest.NewRequest("POST", "/v1/nodes/register", bytes.NewBufferString("{invalid"))
...
func TestRegisterInvalidCapabilityVersion(t *testing.T) {
	handler := &NodeHandler{registrationPSK: "test-psk"}
...
func TestRegisterMissingNodeSlug(t *testing.T) {
	handler := &NodeHandler{registrationPSK: "test-psk"}
	...
	body := NodeRegistrationRequest{
		...
		Capability: NodeCapabilityReport{
			Version: 1,
			Node:    NodeCapabilityNode{NodeSlug: ""}, // Empty slug
		},
	}
	jsonBody, _ := json.Marshal(body)
	req := httptest.NewRequest("POST", "/v1/nodes/register", bytes.NewBuffer(jsonBody))
