...
- **9190** - Worker API - Run jobs on the node (worker node); chosen to avoid conflict with common 8xxx ports and other apps
...
When nodes run on the host, that is typically <http://host.containers.internal:9190>.

## Worker Node

- **Worker API:** listens `:9190` by default.
  This default is unique to CyNodeAI to avoid conflict with User API Gateway (8080), control-plane (8082), and other common 8xxx services.
...
- **Single-host dev:** Ports 5432, 8080, 8082, 9190, 11434 must be free for a full stack (orchestrator + one node + Ollama).
  Optional: 8083, 8084.
- **Orchestrator-only (docker compose):** 5432, 8080, 8082, 11434.
  Node runs elsewhere and uses 9190 (and optionally 11434 for node-local Ollama).
- **Multiple nodes on one host:** Each node's Worker API needs a distinct port (e.g. 9190, 9191, 9192).
...
- **Worker node:** Worker API 9190 (or `NODE_PORT` in dev-setup); inference proxy in sandbox pods exposes `http://localhost:11434` to the sandbox per [Inference (Ollama and Proxy)](#inference-ollama-and-proxy).
...
- **Worker API** - Default: :9190 - Override: `LISTEN_ADDR`; node YAML `worker_api.listen_port`
