...
// testGetAgentTokenFailsWithCorruptKEMKeystore writes content to .kem_keystore.enc and expects GetAgentToken to fail.
func testGetAgentTokenFailsWithCorruptKEMKeystore(t *testing.T, kemContent []byte, failMsg string) {
	t.Helper()
	testFIPSModeKnownOff = true
	defer func() { testFIPSModeKnownOff = true }()
	t.Setenv(masterKeyEnvName, validMasterKeyB64())
	...
	if err := os.WriteFile(kemPath, kemContent, 0o600); err != nil {
...
func TestGetAgentToken_CorruptKEMKeystore(t *testing.T) {
	testGetAgentTokenFailsWithCorruptKEMKeystore(t,
		[]byte(`{"version":99,"algorithm":"AES-256-GCM","nonce_b64":"","payload_b64":""}`),
		"expected GetAgentToken to fail when kem keystore is corrupt")
}
...
func TestGetAgentToken_KEMKeystoreInvalidJSON(t *testing.T) {
	testGetAgentTokenFailsWithCorruptKEMKeystore(t,
		[]byte("not valid json"),
		"expected GetAgentToken to fail when kem keystore is invalid JSON")
}
