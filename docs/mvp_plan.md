...
This document is the single comprehensive MVP development plan as of 2026-03-12.
It reflects the current tech specs, the accepted UDS-based container inference contract, and the latest implemented MVP status.
...
- **Phase 2** - MCP in the loop, workflow, and SBA orchestration - In progress; several foundations are already implemented
- **Phase 3** - Multi-node selection, leases, retries, telemetry - Not started as a dedicated phase, though some supporting telemetry/workflow pieces landed early
- **Phase 4** - API Egress, Secure Browser, external model routing, CLI expansion, admin console after CLI - Not started as a dedicated phase, though small enabling slices landed early

## Scope Summary

- **Inference in sandbox** - Worker-managed inference proxy path over UDS; sandbox receives `INFERENCE_PROXY_URL=http+unix://...` (or equivalent socket-path env), not `OLLAMA_BASE_URL`; E2E scenario runs a task invoking inference from inside the sandbox.
...
- **Phase 1.5:** Complete.
  CLI (cynork), worker-managed sandbox inference path, and prompt interpretation: `input_mode` (prompt/script/commands), default prompt-as-model path, and BDD/feature coverage are in place.
...
- **CI/E2E:** `just ci` and `just e2e --stop-on-success` have been verified passing in recent local validation.
...
**Current state:** Prompt->model behavior is implemented for the MVP single-node slice.
Prompt-mode tasks are treated as interpreted requests rather than literal shell.
Depending on the configured execution path, the system may satisfy prompt mode through orchestrator-side inference or a worker-mediated path, while explicit `input_mode` `"script"` or `"commands"` remains literal shell execution for backward compatibility.
...
- **P1.5-02 (4-8h): Minimal "prompt as model input" execution path with result = model output.**
  ...
  - **Option A**: Dispatch a sandbox job with a fixed model-call command that uses the worker-provided UDS inference proxy URL and prints the response to stdout.
...
- **Node-local inference from sandbox:** For each job that may use inference, the worker provides a container-facing inference path over UDS.
  The sandbox receives an inference proxy URL such as `INFERENCE_PROXY_URL=http+unix://...` (or an equivalent socket-path env), not a TCP loopback endpoint.
  The proxy path enforces request size and timeout limits and MUST NOT expose credentials.
...
- **Implementation ownership:** Worker node (Node Manager or Worker API) provides the UDS inference path and injects the worker-managed inference proxy env into the sandbox.
...
- **E2E:** Scenario that runs task with inference inside the sandbox and validates the UDS-based contract rather than a TCP loopback URL.
...
Worker BDD: GET /readyz, 413 on oversized body.
`just ci` passes.
...
- **`features/e2e/single_node_happy_path.feature`:** Inference scenario exists (`@inference_in_sandbox`) and reflects the current UDS-based container inference contract.
  Script-driven `just e2e` remains the primary end-to-end path.
...
- **BDD suites:** `just test-bdd` runs module `_bdd` suites that currently include orchestrator, worker_node, cynork, agents, and e2e support modules; all pass under `just ci`.
...
- Inference proxy and container-boundary routing: worker node supports `use_inference: true` jobs with a worker-managed inference path to the sandbox over UDS (`INFERENCE_PROXY_URL` / `http+unix` contract).
...
- E2E and OpenAI chat: Test 5d in `run_e2e_test` (GET /v1/models, POST /v1/chat/completions with `cynodeai.pm`); routing to PMA is worker-mediated (Worker API proxy) and PMA inference connectivity is specified by the orchestrator start bundle; chat routing in `openai_chat.go` aligned with openai_compatible_chat_api.md (effective model, PMA vs direct inference).
  `just e2e` passes.
...
This section summarizes MVP remediation status and suggested next work.
...
The basic E2E path for SBA job results is also in place.
