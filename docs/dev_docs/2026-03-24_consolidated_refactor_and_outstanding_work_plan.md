...
- No code changes in this conversation (docs-only); this plan is for future execution.
...
- [ ] Inspect current `orchestrator/internal/models` and `orchestrator/internal/database` for existing `TaskArtifact` model and any artifact-related store methods.
- [ ] Inspect `orchestrator/docker-compose.yml` for current services; confirm MinIO (or equivalent S3) is not yet present.
- [ ] Inspect `orchestrator/internal/handlers/` for any existing artifact endpoints; identify what must be added or refactored.
- [ ] Inspect MCP gateway (`orchestrator/internal/mcpgateway/`) for existing artifact tool handlers; identify gaps vs spec.
- [ ] Identify the GORM record struct pattern from the completed GORM table definition plan and confirm new artifact models follow it (domain base in `models`, record in `database`).
- [ ] Review the "Deferred Implementation" section in the spec and map each item to implementation files and tests.
...
- [ ] **Python E2E tests** (add or update first so spec-defined behavior is locked):
  - [ ] Add or create a dedicated artifact E2E module (e.g. `e2e_0850_artifacts_crud.py`) covering: Create with scope, Read with RBAC, Update blob overwrite, Delete with vector cleanup, Find/list with scope filters and pagination.
  - [ ] Add E2E for MCP artifact tools (`artifact.put`, `artifact.get`, `artifact.list`) via PMA chat or direct MCP call.
  - [ ] Run `just e2e` for the new module and confirm tests fail before implementation.
- [ ] **BDD scenarios** (add or update in `features/orchestrator/` or `features/e2e/`):
  - [ ] Add scenarios for artifact CRUD API with scope-based access (user, group, project, global).
  - [ ] Add scenarios for RBAC enforcement: owner access, cross-principal read via grant, deny by default.
  - [ ] Add scenarios for MCP artifact tool routing (PMA/PAA allowlists).
- [ ] **Go unit tests** (add failing tests in orchestrator packages):
  - [ ] Artifact metadata model and GORM record struct (domain base + record embedding `GormModelUUID`).
  - [ ] Store methods: `CreateArtifact`, `GetArtifactByID`, `UpdateArtifact`, `DeleteArtifact`, `ListArtifactsByScope`.
  - [ ] S3 client wrapper: upload, download, delete, overwrite operations.
  - [ ] RBAC enforcement: scope-based read/write/delete permissions per spec.
  - [ ] Handler tests for all five REST endpoints (`POST`, `GET /{id}`, `PUT /{id}`, `DELETE /{id}`, `GET /`).
  - [ ] MCP tool handler tests for `artifact.put`, `artifact.get`, `artifact.list` in the MCP gateway.
- [ ] Run all three test layers and confirm they fail for the correct reasons.
- [ ] Validation gate: do not proceed until failing tests across all three layers prove the gap.

#### Green (Task 1)

- [ ] Add MinIO (or equivalent S3-compatible service) to `orchestrator/docker-compose.yml` with port 9000, volume, and env wiring (`ARTIFACTS_S3_ENDPOINT`, `ARTIFACTS_S3_ACCESS_KEY`, `ARTIFACTS_S3_SECRET_KEY`, `ARTIFACTS_S3_BUCKET`).
- [ ] Implement S3 client wrapper in `orchestrator/internal/` (upload, download, delete, overwrite) using an MIT/Apache-2.0 licensed Go S3 SDK.
- [ ] Define artifact domain base struct and `ArtifactRecord` in `orchestrator/internal/database` following GORM model structure standard.
  - [ ] Fields: `scope_level`, `owner_user_id`, `group_id`, `project_id`, `path`, `storage_ref`, `size_bytes`, `content_type`, `checksum_sha256`, `created_by_job_id`, `last_modified_by_job_id`, `correlation_task_id`, `run_id`.
  - [ ] Unique constraints per `scope_level` partition per spec.
- [ ] Register `ArtifactRecord` in `migrate.go`.
- [ ] Implement Store methods for artifact CRUD with scope-based queries and RBAC evaluation.
- [ ] Implement handler functions for all five REST endpoints per the spec algorithms.
  - [ ] RBAC enforcement on every operation using subject model from `rbac_and_groups.md`.
  - [ ] Vector items cleanup on delete per spec.
- [ ] Implement MCP tool handlers (`artifact.put`, `artifact.get`, `artifact.list`) in the MCP gateway; route through same backend and RBAC as REST.
- [ ] Implement artifact hashing: small artifacts hashed on upload; large artifacts deferred to background job.
- [ ] Implement stale artifact cleanup (configurable, disabled by default) per spec.
- [ ] Run targeted tests until they pass.
- [ ] Validation gate: do not proceed until artifacts CRUD, RBAC, MCP, and S3 integration are green.

#### Refactor (Task 1)

- [ ] Extract shared RBAC evaluation helpers if duplicated between artifact handlers and other scope-based handlers.
- [ ] Ensure S3 client is testable (interface-based) and supports mock backends for unit tests.
...
- [ ] **Go unit tests:** Run `just test-go-cover` for orchestrator (database, handlers, mcpgateway, S3 client packages); confirm all artifact unit tests pass and coverage meets thresholds.
- [ ] **BDD tests:** Run `just test-bdd` for artifact-related BDD scenarios; confirm all scenarios pass.
- [ ] **Python E2E tests:** Run `just setup-dev restart --force` then `just e2e --tags artifacts` (or targeted module); confirm all artifact E2E tests pass.
- [ ] Run `just lint-go` and `just lint-go-ci` for changed packages.
- [ ] Run `just docs-check` if any spec or README changed.
- [ ] Validation gate: do not start Task 2 until all three test layers and lint checks pass.
...
- [ ] Validation gate: do not start Task 3 until all three test layers and lint checks pass.
...
- [ ] Run all three test layers and confirm failures or mismatches.
- [ ] Validation gate: do not proceed until test gaps are proven across all three layers.
...
- [ ] Validation gate: do not start Task 4 until all three test layers and lint checks pass.
...
- [ ] Run all three test layers and confirm failures match the known Bug 5 symptoms.
- [ ] Validation gate: do not proceed until root cause is confirmed and failing tests prove the gap.
...
- [ ] Run `just lint-python` for changed test scripts.
- [ ] Validation gate: do not start Task 5 until all three test layers and lint checks pass.
...
- [ ] Run all three test layers and confirm they fail for the correct reasons.
- [ ] Validation gate: do not proceed until the gaps are proven across all three layers.
...
- [ ] Run `just lint-go` for changed packages.
- [ ] Validation gate: do not start Task 6 until all three test layers and lint checks pass.
...
- [ ] Run all three test layers and confirm failures for correct reasons.
- [ ] Validation gate: do not proceed until gateway gaps are proven across all three layers.
...
- [ ] Run `just lint-go` for changed packages.
- [ ] Validation gate: do not start Task 7 until all three test layers and lint checks pass.
...
- [ ] Run all three test layers and confirm they fail.
- [ ] Validation gate: do not proceed until TUI streaming UX gap is proven across all three layers.
...
- [ ] Validation gate: do not start Task 8 until all three test layers and lint checks pass.
...
- [ ] Validation gate: do not proceed until BDD step strategy is clear and all three layers reflect current state.
...
- [ ] **Python E2E tests:** Run `just setup-dev restart --force` then run `just e2e --tags pma_inference`, `just e2e --tags chat`, `just e2e --tags tui_pty`; confirm all streaming E2E files pass.
  Run full `just e2e` and confirm no regressions.
- [ ] Validation gate: do not start Task 9 until all three test layers pass with no regressions.
...
- [ ] Run all three test layers and confirm they fail for the right reason.
- [ ] Validation gate: do not proceed until failing tests across all three layers prove the gap.
...
- [ ] Validation gate: do not start Task 10 until all three test layers and full CI pass.
...
- [ ] Run all three test layers per slice and confirm they fail before implementation.
- [ ] Validation gate: do not proceed until test plan is defined and failing tests exist for each slice across all three layers.
...
- [ ] Run `just ci` and full `just e2e` for regression check.
- [ ] Validation gate: do not start Task 11 until all three test layers and full CI pass.
...
- [ ] Validation gate: do not start Task 12 until all checks pass.
...
- [ ] Validation gate: plan complete only when all three test layers, CI, and docs-check pass.
