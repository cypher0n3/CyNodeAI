...
class TestProxyPmaFunctional(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        env_worker["WORKER_INTERNAL_LISTEN_ADDR"] = (
            f"127.0.0.1:{config.PROXY_PMA_TEST_WORKER_PORT + 1000}"
        )
    ...
    def test_proxy_forwards_chat_completion(self):
        ...
        for _ in range(6):
            ...
            if code == 200:
                break
            # Under full-suite load PMA can take an extra moment before chat route is reachable.
            time.sleep(1.5)
        self.assertEqual(code, 200, f"proxy returned {code}: {resp_body!r}")
        status, raw = parse_proxy_response(resp_body)
...
# --- Mock inference (Ollama-compatible /api/generate and /api/chat) for PMA test ---


def _make_mock_inference_handler(response_text):
    """Return a handler class for Ollama-style inference endpoints.

    Handles both /api/generate (legacy) and /api/chat (current PMA callInference path).
    """
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def log_message(self, format, *args):  # pylint: disable=redefined-builtin  # noqa: F841
            pass
...
class TestProxyPmaWithInference(unittest.TestCase):
    ...
    Starts: mock Ollama-compatible server (POST /api/generate, /api/chat) -> PMA
    (OLLAMA_BASE_URL=mock, no MCP gateway) -> worker-api proxy.
    Asserts proxy -> PMA -> inference path returns 200 with content.
    """
    ...
    def setUpClass(cls):
        ...
        # Mock inference (Ollama /api/generate + /api/chat); port 0 avoids "Address already in use"
        ...
        time.sleep(0.2)
        # Verify mock inference is up via /api/generate health-check
        code, _ = helpers.run_curl_with_status(
            "POST",
        ...
        env_worker["WORKER_INTERNAL_LISTEN_ADDR"] = f"127.0.0.1:{worker_port + 1000}"
        env_worker["WORKER_API_STATE_DIR"] = state_dir
        with _popen_keepalive(
            [config.WORKER_API_BIN],
            cwd=config.PROJECT_ROOT,
...
def _ollama_ensure_model(runtime, container_name, model, ollama_base_url=None):
    """Pull model if not listed, warm up via HTTP; return True if model is available."""
    r = subprocess.run(
    ...
    if model not in (r.stdout or ""):
        ...
        if r.returncode:
            return False
    # Warm up: load model via direct HTTP /api/chat so first PMA chat does not timeout.
    # Avoids `ollama run` CLI which times out after 60 s for cold-start models like qwen3.5.
    if ollama_base_url:
        payload = json.dumps({
            "model": model,
        ...
        try:
            ...
            pass  # warm-up failure is non-fatal; PMA will handle first-request latency
    return True
...
class TestProxyPmaWithRealOllama(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        if not _ollama_ensure_model(
            cls._runtime, container_name, config.OLLAMA_E2E_MODEL, ollama_url
        ):
            _stop_ollama_container(cls._runtime, container_name)
            cls._ollama_container = None
...
def _run_curl_unix_with_status(socket_path, url_path, data=None, headers=None, timeout=30):
    """Run curl over unix socket; return (status_code, body)."""
    cmd = [
        "curl",
        "-s",
        "-w",
...
class TestManagedAgentInternalProxyUDS(unittest.TestCase):
    ...
    def test_internal_proxy_requires_identity_binding_over_tcp(self):
        ...
        internal_addr = os.environ.get("WORKER_INTERNAL_LISTEN_ADDR", "127.0.0.1:9191").strip()
        payload = build_proxy_request(
            "POST",
            "/v1/mcp/tools/call",
            json.dumps({"tool_name": "tasks.get", "arguments": {}}).encode("utf-8"),
        )
        code, body = helpers.run_curl_with_status(
            "POST",
            f"http://{internal_addr}/v1/worker/internal/orchestrator/mcp:call",
            data=json.dumps(payload),
            headers={"Content-Type": "application/json"},
            timeout=5,
        )
        if not code:
            self.skipTest("worker internal listen address is unreachable in this environment")
        self.assertEqual(code, 401, f"expected 401 missing identity binding, got {code}: {body!r}")
        self.assertIn("missing caller identity binding", body)

    def test_internal_proxy_missing_token_over_per_service_uds(self):
        """Per-service UDS call fails with 401 without service token."""
        if not os.path.isfile(config.WORKER_API_BIN):
            self.skipTest(f"worker-api binary not found: {config.WORKER_API_BIN}")
        state_dir = tempfile.mkdtemp(
            prefix="proxy-pma-internal-missing-token-",
            dir=os.path.join(config.PROJECT_ROOT, "tmp"),
        )
        os.makedirs(state_dir, 0o700, exist_ok=True)
        service_id = "svc-test"
        worker_bearer = base64.standard_b64encode(os.urandom(18)).decode("ascii")
        node_config = {
            "version": 1,
            "config_version": "1",
            "issued_at": "2026-03-09T00:00:00Z",
            "node_slug": "dev-node-1",
            "orchestrator": {"base_url": "http://127.0.0.1:12082"},
            "worker_api": {"orchestrator_bearer_token": worker_bearer},
            "managed_services": {
                "services": [
                    {
                        ...
                        "image": "pma:latest",
                        "orchestrator": {
                            "mcp_gateway_proxy_url": "auto",
                            "ready_callback_proxy_url": "auto",
                        },
                    }
                ]
            },
        ...
        env = os.environ.copy()
        env["WORKER_API_BEARER_TOKEN"] = worker_bearer
        env["WORKER_API_STATE_DIR"] = state_dir
        env["WORKER_NODE_CONFIG_JSON"] = json.dumps(node_config)
        env["CYNODE_SECURE_STORE_MASTER_KEY_B64"] = (
            base64.standard_b64encode(os.urandom(32)).decode("ascii")
        )
        env["LISTEN_ADDR"] = "127.0.0.1:0"
        env["WORKER_INTERNAL_LISTEN_ADDR"] = "127.0.0.1:0"
        with subprocess.Popen(
            ...
            try:
                sock = os.path.join(
                    state_dir, "run", "managed_agent_proxy", service_id, "proxy.sock"
                )
                ...
                if not os.path.exists(sock):
                    ...
                    if proc.stderr is not None:
                        try:
                            err_text = (proc.stderr.read() or b"").decode(
                                "utf-8", errors="replace"
                            )
                        except AttributeError:
                            err_text = proc.stderr.read() or ""
                    self.skipTest(
                        "worker-api did not create per-service proxy.sock in time; "
                        f"stderr={err_text!r}"
                    )
                payload = build_proxy_request(
                    "POST",
                    "/v1/mcp/tools/call",
                    json.dumps(
                        {"tool_name": "tasks.get", "arguments": {"task_id": "nonexistent"}}
                    ).encode("utf-8"),
                )
                code, body = _run_curl_unix_with_status(
                    sock,
                    "/v1/worker/internal/orchestrator/mcp:call",
                    data=json.dumps(payload),
                    headers={"Content-Type": "application/json"},
                    timeout=10,
                )
                self.assertEqual(
                    code,
                    401,
                    f"expected 401 token unavailable over per-service UDS, got {code}: {body!r}",
                )
                self.assertIn("agent token unavailable for caller identity", body)
            finally:
                proc.terminate()
                proc.wait(timeout=5)

    def test_internal_proxy_per_service_uds_forwards_when_token_exists(self):
        """Per-service UDS call forwards through worker proxy in a running dev stack."""
        candidates = []
        if config.NODE_STATE_DIR:
            candidates.append(
                os.path.join(
        ...
        socket_path = next((p for p in candidates if os.path.exists(p)), "")
        if not socket_path:
            self.skipTest("no running dev-stack managed-agent proxy socket found for pma-main")
        payload = build_proxy_request(
            "POST",
            "/v1/mcp/tools/call",
            json.dumps(
        ...
        self.assertEqual(
            code, 200, f"expected wrapped proxy response over UDS, got {code}: {body!r}"
        )
        wrapped = helpers.parse_json_safe(body)
        self.assertIsInstance(wrapped, dict, f"expected JSON proxy envelope, got: {body!r}")
        self.assertIn("status", wrapped, f"expected upstream status in envelope, got: {wrapped!r}")
