...
type cynorkState struct {
	...
	sessionGateway string            // set by "the TUI is running with gateway" step
	...
	// Task 3 tracking fields for slash command and gateway verification.
	modelsErrorMode       bool              // GET /v1/models returns 500
	modelIDs              []string          // GET /v1/models returns these ids
	...
	taskArtifactsByID     map[string][]string
...
func InitializeCynorkSuite(sc *godog.ScenarioContext, state *cynorkState) {
	sc.Before(func(ctx context.Context, _ *godog.Scenario) (context.Context, error) {
		...
		state.taskArtifactsByID = nil
		state.nodesByID = nil
		state.projectsByID = nil
	...
	InitializeCynorkSuiteExtra(sc, state)
}
