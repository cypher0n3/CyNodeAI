...
Scenario: Worker API rejects request without bearer token
...
@req_worker_0102
@spec_cynai_worker_workerauth
Scenario: Worker API rejects request with invalid bearer token
...
Scenario: Sandbox receives UDS inference proxy URL when job requests inference
  Given the worker API is configured with a valid bearer token
  When I submit a sandbox job with use_inference that runs command "echo $INFERENCE_PROXY_URL"
  Then the sandbox job completes successfully
  And the sandbox job result stdout contains "http+unix://"
