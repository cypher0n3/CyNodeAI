...
// TestCreateTaskWithValidBody tests CreateTask with valid request body
func TestCreateTaskWithValidBody(t *testing.T) {
	handler := &TaskHandler{}
...
// TestLogoutWithTokenButNilDB tests logout with token and nil db
func TestLogoutWithTokenButNilDB(t *testing.T) {
	handler := &AuthHandler{}

	body := LogoutRequest{RefreshToken: "some-refresh-token"}
	jsonBody, _ := json.Marshal(body)
	...
	// Will panic due to nil db when trying to get session
	defer func() {
		_ = recover()
	}()
...
func TestValidateLoginCredentialsStructure(t *testing.T) {
	...
	if result.user.Handle != "testuser" {
		t.Errorf("expected handle 'testuser', got %s", result.user.Handle)
	}
}
