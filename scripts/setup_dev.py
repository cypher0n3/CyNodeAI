...
from scripts import setup_dev_components
...
def show_help():
    ...
    print("  clean           Stop all services; remove postgres and ollama data (if managed)")
    print("  component       Per-component start/stop/restart/rebuild (see below)")
    ...
    print("")
    print("  component <name> <action>  Run action for one component.")
    print("    name:  postgres | control-plane | user-gateway | mcp-gateway | api-egress |")
    print("           ollama | node-manager")
    print("    action:  start | stop | restart | rebuild")
    print("    Rebuild also: cynode-pma | worker-api | inference-proxy | cynode-sba.")
    print("    Use --no-cache to pass --no-cache to podman/docker build.")
    print("    Example: setup_dev.py component node-manager restart")
    print("    Example: setup_dev.py component control-plane rebuild")
    print("")
...
def _run_clean():
    """Stop all services, remove postgres container/volume, and ollama data volume if managed."""
    setup_dev_impl.stop_all()
    setup_dev_impl.clean_ollama_data()
    return setup_dev_impl.clean_postgres()


def _run_component(args):
    """Run component <name> <action>. Returns 0 on success, 1 on failure."""
    extra = getattr(args, "extra", []) or []
    if len(extra) < 2:
        print("Usage: setup_dev.py component <name> <action>", file=sys.stderr)
        print("  name: postgres | control-plane | user-gateway | mcp-gateway |", file=sys.stderr)
        print("        api-egress | ollama | node-manager; rebuild: cynode-pma |", file=sys.stderr)
        print("        worker-api | inference-proxy | cynode-sba", file=sys.stderr)
        print("  action: start | stop | restart | rebuild", file=sys.stderr)
        return 1
    name, action = extra[0], extra[1]
    if action not in ("start", "stop", "restart", "rebuild"):
        print(f"Invalid action: {action}. Use start | stop | restart | rebuild", file=sys.stderr)
        return 1
    force = getattr(args, "force", False) or bool(os.environ.get("SETUP_DEV_FORCE_BUILD"))
    no_cache = getattr(args, "no_cache", False)
    if action == "start":
        opts = type("Opts", (), {"extra_env": _default_inference_env()})()
        return 0 if setup_dev_components.component_start(name, extra_env=opts.extra_env) else 1
    if action == "stop":
        return 0 if setup_dev_components.component_stop(name) else 1
    if action == "restart":
        opts = type("Opts", (), {"extra_env": _default_inference_env()})()
        return 0 if setup_dev_components.component_restart(name, extra_env=opts.extra_env) else 1
    if action == "rebuild":
        ok = setup_dev_components.component_rebuild(name, force=force, no_cache=no_cache)
...
def main():
    ...
    parser.add_argument(
        ...
        choices=[
            ...
            "start", "stop", "restart", "clean", "component", "test-e2e", "full-demo", "help",
        ],
        help="Command to run",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--force",
        action="store_true",
        help="For component rebuild: skip cache and rebuild image/binary",
    )
    parser.add_argument(
        "--no-cache",
        action="store_true",
        help="For component rebuild: pass --no-cache to podman/docker build (no layer cache)",
    )
