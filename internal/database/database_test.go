...
func TestGetUserByHandleNotFound(t *testing.T) {
	db, mock := newMockDB(t)
	defer db.Close()
	...
	_, err := db.GetUserByHandle(context.Background(), "nonexistent")
...
func TestGetPasswordCredentialByUserIDNotFound(t *testing.T) {
	db, mock := newMockDB(t)
	defer db.Close()
	...
	_, err := db.GetPasswordCredentialByUserID(context.Background(), userID)
	if !errors.Is(err, ErrNotFound) {
		t.Errorf("expected ErrNotFound, got %v", err)
	}
}
...
func TestInvalidateRefreshSession(t *testing.T) {
	db, mock := newMockDB(t)
	defer db.Close()

	sessionID := uuid.New()
	mock.ExpectExec(`UPDATE refresh_sessions SET is_active = false`).
		WithArgs(sessionID, sqlmock.AnyArg()).
		WillReturnResult(sqlmock.NewResult(0, 1))

	err := db.InvalidateRefreshSession(context.Background(), sessionID)
	if err != nil {
...
func TestInvalidateAllUserSessions(t *testing.T) {
	...
	err := db.InvalidateAllUserSessions(context.Background(), userID)
	if err != nil {
		t.Fatalf("InvalidateAllUserSessions failed: %v", err)
	}
}
...
func TestGetPasswordCredentialError(t *testing.T) {
	db, mock := newMockDB(t)
	defer db.Close()
...
func TestInvalidateRefreshSessionError(t *testing.T) {
	db, mock := newMockDB(t)
	defer db.Close()

	sessionID := uuid.New()
	mock.ExpectExec(`UPDATE refresh_sessions`).
		WillReturnError(errors.New("db error"))
...
func TestInvalidateAllUserSessionsError(t *testing.T) {
	db, mock := newMockDB(t)
	defer db.Close()
...
func TestGetUserByHandleError(t *testing.T) {
	db, mock := newMockDB(t)
	defer db.Close()
	...
	_, err := db.GetUserByHandle(context.Background(), "user")
	if err == nil {
		t.Fatal("expected error, got nil")
	}
}
