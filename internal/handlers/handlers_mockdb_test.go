...
import (
        ...
        "github.com/cypher0n3/cynodeai/internal/testutil"
)
...
func TestAuthHandler_LoginInactiveUser(t *testing.T) {
        mockDB := testutil.NewMockDB()
        jwtMgr := auth.NewJWTManager("test-secret-key-1234567890123456", 15*time.Minute, 7*24*time.Hour, 24*time.Hour)
        rateLimiter := auth.NewRateLimiter(100, time.Minute)
        logger := newTestLogger()
        ...
        body := LoginRequest{Handle: "inactiveuser", Password: "password"}
        jsonBody, _ := json.Marshal(body)
        req := httptest.NewRequest("POST", "/v1/auth/login", bytes.NewBuffer(jsonBody))
        rec := httptest.NewRecorder()
...
func TestAuthHandler_LoginNoPasswordCredential(t *testing.T) {
        mockDB := testutil.NewMockDB()
        jwtMgr := auth.NewJWTManager("test-secret-key-1234567890123456", 15*time.Minute, 7*24*time.Hour, 24*time.Hour)
        rateLimiter := auth.NewRateLimiter(100, time.Minute)
        logger := newTestLogger()
        ...
        user := &models.User{
                ID:        uuid.New(),
                Handle:    "nopassworduser",
                IsActive:  true,
                CreatedAt: time.Now().UTC(),
                UpdatedAt: time.Now().UTC(),
        }
        mockDB.AddUser(user)
        // No password credential added

        body := LoginRequest{Handle: "nopassworduser", Password: "password"}
        jsonBody, _ := json.Marshal(body)
        req := httptest.NewRequest("POST", "/v1/auth/login", bytes.NewBuffer(jsonBody))
        rec := httptest.NewRecorder()
...
func TestTaskHandler_CreateTaskDBError(t *testing.T) {
        ...
        userID := uuid.New()
        ctx := context.WithValue(context.Background(), contextKeyUserID, userID)
        body := CreateTaskRequest{Prompt: "test prompt"}
        jsonBody, _ := json.Marshal(body)
        req := httptest.NewRequest("POST", "/v1/tasks", bytes.NewBuffer(jsonBody)).WithContext(ctx)
        rec := httptest.NewRecorder()
...
func (m *sessionErrorMockDB) CreateRefreshSession(_ context.Context, _ uuid.UUID, _ []byte, _ time.Time) (*models.RefreshSession, error) {
        if m.failOnCreateSession {
                return nil, errors.New("session creation failed")
        }
        return m.MockDB.CreateRefreshSession(context.Background(), uuid.New(), nil, time.Now())
}
...
// Tests for handleNewNodeRegistration JWT error
type jwtErrorNodeMockDB struct {
        *testutil.MockDB
}

func (m *jwtErrorNodeMockDB) GetNodeBySlug(_ context.Context, slug string) (*models.Node, error) {
        // Return not found to trigger new node registration
