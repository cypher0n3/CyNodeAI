...
As of 2026-03-12, the core MVP foundation and single-node slices are largely in place:

- Phase 0 foundations are implemented for the MVP-supported surfaces.
- Phase 1, Phase 1.5, and Phase 1.7 are functionally complete for the current single-node product slice.
- Phase 2 is in progress, with MCP gateway scoping/auditing, preference tools, SBA runner integration, and workflow foundations already landed.
- Phase 3 and Phase 4 remain roadmap phases, though a small amount of enabling work has landed early.
...
- **Single-node full capability slice (Phase 1.5)**.
  - Inference from inside the sandbox via a node-local proxy path exposed to containers over UDS.
...
- Enable node-local inference access from inside the sandbox.
  Implement the worker-managed UDS inference proxy approach so sandboxes receive an inference proxy URL such as `INFERENCE_PROXY_URL=http+unix://...` without using a TCP endpoint.
  See [`docs/tech_specs/worker_node.md`](tech_specs/worker_node.md) and [`docs/tech_specs/sandbox_container.md`](tech_specs/sandbox_container.md).
