...
# Build node manager (production binary). Binary: cynodeai-wnm (Worker Node Manager).
build-node-manager: install-go
    @just _build_prod cynodeai-wnm ./worker_node/cmd/node-manager worker_node/bin

# Build node manager (dev, debug symbols). Binary: cynodeai-wnm-dev.
...
# Build all container images (pass extra flags to all builds, e.g. just build-images --no-cache).
build-images *ARGS:
    @just build-control-plane-image {{ ARGS }}
    @just build-user-gateway-image {{ ARGS }}
    @just build-mcp-gateway-image {{ ARGS }}
    @just build-api-egress-image {{ ARGS }}
    @just build-worker-api-image {{ ARGS }}
    @just build-node-manager-image {{ ARGS }}
    @just build-inference-proxy-image {{ ARGS }}
    @just build-cynode-pma-image {{ ARGS }}
    @just build-cynode-sba-image {{ ARGS }}

# --- Container image builds (pass extra flags after the recipe, e.g. just build-worker-api-image --no-cache) ---
# Build control-plane container image (orchestrator stack).
...
# Build user-gateway container image (orchestrator stack).
...
build-worker-api-image *ARGS:
    @just _build_container worker_node/cmd/worker-api/Containerfile cynodeai-worker-api {{ ARGS }}
...
# Build inference-proxy container image (E2E / node inference).
...
# Internal: build a container image from repo root. Pass-through args go to podman/docker build (e.g. --no-cache).
# Usage: just _build_container <containerfile_rel> <tag> [ARGS...]
