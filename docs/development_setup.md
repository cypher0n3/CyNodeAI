...
- **node-manager** (binary: cynodeai-wnm) - (dynamic) - Registers node, reports capabilities
...
```bash
cd worker_node
WORKER_API_BEARER_TOKEN=dev-worker-api-token docker compose up -d   # or podman compose up -d
```
