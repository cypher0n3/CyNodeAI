...
- [Integration With Worker API](#integration-with-worker-api)
  ...
  - [Inference Access and Allowed Models](#inference-access-and-allowed-models)
  - [Worker Proxies Summary](#worker-proxies-summary)
...
### Inference Proxy (Unified UDS)

- When the node provides Ollama inference, the Node Manager runs the sandbox with an **inference proxy** that exposes only a **Unix domain socket** to the sandbox (per [Unified UDS Path](worker_node.md#spec-cynai-worker-unifiedudspath)).
  The node injects an inference proxy URL (e.g. `INFERENCE_PROXY_URL=http+unix://...`) into the sandbox; the node MUST NOT inject `OLLAMA_BASE_URL=http://localhost:11434` or any TCP endpoint.
  Any agent use of inference (e.g. LLM calls from within the sandbox) MUST use this UDS endpoint; the proxy forwards to the node's Ollama and keeps traffic node-local.
...
- The SBA MUST have access to **at least one** model; the runtime (node and/or orchestrator) MUST provide inference via either:
  - **Worker proxy (node-local, UDS):** When the node provides Ollama, the Node Manager injects an inference proxy URL over UDS (e.g. `INFERENCE_PROXY_URL=http+unix://...`); the SBA calls this endpoint using only models from the job's allowed set.
