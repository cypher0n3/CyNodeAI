...
Whenever the CLI outputs JSON (including `--output json`, JSON embedded in chat or table output, or other responses), the JSON MUST be pretty-printed (indented, with newlines) for human readability; see [Pretty-Printed JSON](#pretty-printed-json-output).
...
Output MUST be shown inline in the chat (pretty-printed per [Pretty-Printed JSON Output](#pretty-printed-json-output) when the output is JSON).
