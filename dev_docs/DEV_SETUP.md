...
- **Go coverage**: Run `just test-go-cover-podman` (or `just ci`).
  For the orchestrator, this starts a Postgres container with Podman (`pgvector/pgvector:pg16` on port 15432), sets `POSTGRES_TEST_DSN`, runs tests, then removes the container.
  No Docker socket or testcontainers needed.
