...
func pollInferencePathUntilReady(ctx context.Context, store database.Store, logger *slog.Logger, pollInterval time.Duration) bool {
	for {
		...
		ok, err := readinesscheck.InferencePathAvailable(ctx, store)
		if err != nil {
			if logger != nil {
...
func readyzHandler(store database.Store, cfg *config.OrchestratorConfig, logger *slog.Logger) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		ctx := r.Context()
		hasPath, err := readinesscheck.InferencePathAvailable(ctx, store)
		if err != nil {
			if logger != nil {
		...
		localReady := cfg != nil && cfg.PMAEnabled && readinesscheck.PMASubprocessReady(ctx, cfg.PMAListenAddr)
		workerReportedReady := readinesscheck.HasWorkerReportedPMAReady(ctx, store)
