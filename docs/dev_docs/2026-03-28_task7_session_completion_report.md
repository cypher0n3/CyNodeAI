...
**Status:** **Partially complete.** Core PTY harness helpers, gateway/chat/TUI streaming plumbing, coverage fixes, and several E2E/Go tests landed. Some plan lines remain open (bounded auto-reconnect in TUI, full BDD step implementations, fully green `tui_pty` slice, `just lint-go` file-size gate).
